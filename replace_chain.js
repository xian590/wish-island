...
// We need to handle: obj?.prop, obj?.[expr], obj?.method()
// Approach: simple regex replacement for common patterns

let newJs = jsPart;

// Replace nested patterns iteratively
for (let iter = 0; iter < 20; iter++) {
    const before = newJs;
    
    // Pattern 1: simple property access: expr?.identifier
    // Use a simple regex, but be careful with nested expressions
    // We replace from right to left to handle nesting correctly
    
    // Find all ?. positions in the current JS
    const matches = [];
    let idx = 0;
    while (true) {
        ...
        // Check if this is actually optional chaining (not in a string/comment)
        // Simple heuristic: check if it's followed by an identifier or [
